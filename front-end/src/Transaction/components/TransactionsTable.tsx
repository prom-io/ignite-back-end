...
export const TransactionsTable: FunctionComponent<TransactionsTableProps> = ({
    ...
    if (selectedTransaction) {
        if (selectedTransaction.type === "dataUpload") {
            transactionDetailsDialog = <DataOwnerDetailsDialog onClose={() => setSelectedTransaction(undefined)} dataOwner={selectedTransaction?.dataOwner}/>
        } else {
            transactionDetailsDialog = <DataSaleDetailsDialog onClose={() => setSelectedTransaction(undefined)} transaction={selectedTransaction}/>
        }
    }
        ...
        <Fragment>
            <Grid container spacing={2}>
                <Grid item xs={12}>
                    <Table>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell><b>Service node</b></TableCell>
                                <TableCell><b>Data mart</b></TableCell>
                        ...
                        <TableBody>
                            {transactions.map(transaction => (
                                <TableRow>
                                    ...
                                    <TableCell>{shortenString(transaction.serviceNode, 16)}</TableCell>
                    ...
                    {pending && <CircularProgress color="primary" size={25} className={classes.centered}/>}
                </Grid>
                <Grid item xs={12}>

...
export class HashTagsRetriever {
  ...
  public getHashTagsStringsFromText(text?: string): string[] {
    ...
    return text
      .split(HASH_TAG_REGEXP)
      .filter(
        chunk =>
          HASH_TAG_REGEXP.test(chunk) &&
          !REPEATED_HASH_CHARACTER_REGEXP.test(chunk),
      )
  ...
  public async getHashTagsEntitiesFromText(
    text: string,
    ...
    if (hashTagsStrings.length === 0) {
      ...
      return asyncMap(hashTagsStrings, async hashTagString => {
        let hashTag = await this.hashTagsRepository.findByNameAndLanguage(
          hashTagString,
          language,
        );

...
export class StatusesService {
  ...
  public async createStatus(
    ...
    if (
      createStatusRequest.mediaAttachments &&
      createStatusRequest.mediaAttachments.length
    ) {
    ...
    if (createStatusRequest.referredStatusId) {
      referredStatus = await this.statusesRepository.findById(
        createStatusRequest.referredStatusId,
      );
    ...
    if (
      referredStatus &&
      createStatusRequest.statusReferenceType === StatusReferenceType.REPOST &&
      referredStatus.text.length === 0 &&
      referredStatus.mediaAttachments.length === 0 &&
    ...
    const hashTags = await this.hashTagsRetriever.getHashTagsEntitiesFromText(
      createStatusRequest.status,
      currentUser.preferences && currentUser.preferences.language
  ...
  public async findStatusesByUser(
    ...
    if (!user) {
      throw new HttpException(
        `Could not find user with address ${ethereumAddress}`,
        HttpStatus.NOT_FOUND,
      );
    }
    ...
    if (cursors.maxId) {
      if (cursors.sinceId) {
        ...
        const maxCursor = await this.findStatusEntityById(cursors.maxId);
        statuses = await this.statusesRepository.findByAuthorAndCreatedAtBefore(
          user,
          maxCursor.createdAt,
          paginationRequest,
        );
      ...
      const sinceCursor = await this.findStatusEntityById(cursors.sinceId);
      statuses = await this.statusesRepository.findByAuthorAndCreatedAtAfter(
        user,
        sinceCursor.createdAt,
        paginationRequest,
      );
    } else {
      statuses = await this.statusesRepository.findByAuthor(
        user,
        paginationRequest,
      );
    }

    const statusInfoMap = await this.statusesRepository.getStatusesAdditionalInfoMap(
      statuses,
      currentUser,
    );

    return asyncMap(statuses, status =>
      this.statusesMapper.toStatusResponseByStatusInfo(
        status,
  ...
  public async findCommentsOfStatus(
    statusId: string,
    cursors: FeedCursors,
    currentUser?: User,
  ): Promise<StatusResponse[]> {
    ...
    return asyncMap(statuses, comment =>
      this.statusesMapper.toStatusResponseByStatusInfo(
        comment,

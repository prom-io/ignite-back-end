...
export class TokenExchangeService {
    ...
    async getBalanceInProms(ethAddress: string): Promise<string> {
        ...
        if (response.data.status !== "Success") {
            throw new Error(
                `TokenExchangeService: Error occurred: ${JSON.stringify(
                    response.data,
                )}`,
            );
        }
    ...
    async sendTokens(params: SendTokensParams): Promise<string> {
        const response = await this.axios.post<SendTokensResponse>(
            "/sendtokens",
            {
     ...
     * @returns хэш транзакции перевода
     */
    async sendTokensFromMemezatorPrizeFund(
        params: Omit<SendTokensParams, "privateKeyFrom">,
    ): Promise<string> {
        const response = await this.axios.post<SendTokensResponse>(
            "/sendtokens",
            {
                private_key_from:
                    config.MEMEZATOR_PRIZE_FUND_ACCOUNT_PRIVATE_KEY,
                address_to: params.addressTo,
                amount: params.amount,
            },
        );

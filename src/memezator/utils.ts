...
export function getCronExpressionForMemezatorCompetitionSumminUpCron(): string {
  const lastMidnightInCetConvertedToLocalTime = momentTZ().tz("Asia/Tashkent").hours(0).minutes(0).seconds(0).milliseconds(0).local()
...
export function getCurrentMemezatorContestStartTime(): momentTZ.Moment {
  const lastMidnightInCet = momentTZ().tz("Asia/Tashkent").hours(0).minutes(0).seconds(0).milliseconds(0)

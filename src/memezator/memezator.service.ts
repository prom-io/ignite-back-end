...
import {
    WinnerMemesWithLikes,
    MemeWithLikesAndVotingPowers,
    LikeAndVotingPowerAndReward,
    RewardFractions,
} from "./types";
...
export class MemezatorService extends NestSchedule {
    ...
    async memezatorCompetitionSummingUpCron(): Promise<void> {
        if (
            !config.additionalConfig.memezator.disableCompetitionSummingUpCron
        ) {
            this.logger.log(
                "Memezator competition summing up cron job started",
            );
            await this.startMemezatorCompetitionSummingUp({
                startedInCron: true,
                dryRun: false,
            });
            this.logger.log(
                "Memezator competition summing up cron job finished",
            );
        } else {
            this.logger.warn(
                "Memezator competition summing up cron job is disabled",
            );
        }
    }
    ...
    async startMemezatorCompetitionSummingUp(options: {
        ...
        const formattedCompetitionStartDate = competitionStartDate.format(
            "YYYY.MM.DD",
        );
        ...
        if (!options.dryRun) {
            await this.createStatusesAboutWinners(
                winners,
                rewardPool,
                competitionStartDate.toDate(),
            );
            ...
            const transactions = await this.createTransactions(
                winners,
                memezatorContestResult.id,
            );

            if (config.additionalConfig.memezator.performTransactions) {
                this.logger.info(
                    `startMemezatorCompetitionSummingUp: started to perform transactions`,
                );
                ...
                this.logger.info(
                    `startMemezatorCompetitionSummingUp: finished to perform transactions`,
                );
            } else {
                this.logger.info(
                    `startMemezatorCompetitionSummingUp: transactions performing is disabled`,
                );
            }
        }
    ...
    async calculateWinnersWithLikesAndRewards(
        ...
        const memes = await this.statusesRepository.findContainingMemeHashTagAndCreatedAtBetween(
            competitionStartDate,
            competitionEndDate,
        );
        ...
        for (const meme of memes) {
            ...
            for (const like of likes) {
                ...
                if (votingPower > 1) {
                    ...
                    this.logger.info(
                        `Following like ${like.id} is skipped because user ${like.user.id} has not enough voting power.`,
                    );
                }
            }
            ...
            if (!firstPlace || votes > firstPlace.meme.favoritesCount) {
                ...
                !secondPlace ||
                votes > secondPlace.meme.favoritesCount
            ) {
                thirdPlace = secondPlace;
        ...
        if (firstPlace) {
            await this.calculateAndAssignRewardsToVoters(
                firstPlace,
                rewardPool,
                rewardFractionsForFirstPlace,
            );
    ...
    /**
     * WARNING!!!!
     * Есть копия этого метода в сервисе UsersService. Она полностью идентична этому.
     * Копию пришлось сделать из-за проблем с circular dependency (Не смог использовать MemezatorService в UsersService).
     * Поэтому, пока не пофиксим проблему с цикличными зависимостями, нкжн поддерживать идентичность этих 2 одинаковых методов,
     * иными словами - при изменении одного метода, нужно изменять и другого.
     * 
    ...
    private async calculateAndAssignRewardsToVoters(
        ...
        memeWithLikesAndVotingPowers.rewardForAuthor =
            rewardPool * rewardFractions.rewardFractionForAuthor;
        ...
        await asyncForEach(
            ...
            async (likeWithVotingPowerAndReward, i) => {
                ...
                const rewardSumForEveryTicket =
                    rewardForEveryTicket *
                    likeWithVotingPowerAndReward.votingPower;
                likeWithVotingPowerAndReward.reward = rewardSumForEveryTicket;
                likeWithVotingPowerAndReward.rewardsDetailed.push({
                    for: "rewardSumForEveryTicket",
                    reward: rewardSumForEveryTicket,
                });
        ...
        const rewardForEvery2ndRandomTicket =
            (rewardPool *
                rewardFractions.rewardFractionToEqualyShareBetweenEvery2ndRandomVoter) /
            _.sum(_.values(indexesOfEvery2ndRandomTicket));

        await setImmediatePromise();

        const rewardForEvery4thRandomTicket =
            (rewardPool *
                rewardFractions.rewardFractionToEqualyShareBetweenEvery4thRandomVoter) /
            _.sum(_.values(indexesOfEvery4thRandomTicket));

        await setImmediatePromise();

        const rewardForEvery20thRandomTicket =
            (rewardPool *
                rewardFractions.rewardFractionToEqualyShareBetweenEvery20thRandomVoter) /
            _.sum(_.values(indexesOfEvery20thRandomTicket));
         ...
         */
        _.forEach(
            indexesOfEvery2ndRandomTicket,
            (howManyTimesGetsRewardForEvery2ndRandomTicket, index) => {
                const rewardSumForEvery2ndRandomTicket =
                    howManyTimesGetsRewardForEvery2ndRandomTicket *
                    rewardForEvery2ndRandomTicket;
                memeWithLikesAndVotingPowers.likesWithVotingPowersAndRewards[
                    +index
                ].reward += rewardSumForEvery2ndRandomTicket;
                memeWithLikesAndVotingPowers.likesWithVotingPowersAndRewards[
                    +index
                ].rewardsDetailed.push({
                    for: "rewardSumForEvery2ndRandomTicket",
                    reward: rewardSumForEvery2ndRandomTicket,
                });
            },
        );
        ...
        _.forEach(
            indexesOfEvery4thRandomTicket,
            (howManyTimesGetsRewardForEvery4thRandomTicket, index) => {
                const rewardSumForEvery4thRandomTicket =
                    howManyTimesGetsRewardForEvery4thRandomTicket *
                    rewardForEvery4thRandomTicket;
                memeWithLikesAndVotingPowers.likesWithVotingPowersAndRewards[
                    +index
                ].reward += rewardSumForEvery4thRandomTicket;
                memeWithLikesAndVotingPowers.likesWithVotingPowersAndRewards[
                    +index
                ].rewardsDetailed.push({
                    for: "rewardSumForEvery4thRandomTicket",
                    reward: rewardSumForEvery4thRandomTicket,
                });
            },
        );
        ...
        _.forEach(
            indexesOfEvery20thRandomTicket,
            (howManyTimesGetsRewardForEvery20thRandomTicket, index) => {
                const rewardSumForEvery20thRandomTicket =
                    howManyTimesGetsRewardForEvery20thRandomTicket *
                    rewardForEvery20thRandomTicket;
                memeWithLikesAndVotingPowers.likesWithVotingPowersAndRewards[
                    +index
                ].reward += rewardSumForEvery20thRandomTicket;
                memeWithLikesAndVotingPowers.likesWithVotingPowersAndRewards[
                    +index
                ].rewardsDetailed.push({
                    for: "rewardSumForEvery20thRandomTicket",
                    reward: rewardSumForEvery20thRandomTicket,
                });
            },
        );
        ...
        memeWithLikesAndVotingPowers.threeLikesWithVotingPowersAndRewardsWithBiggestRewards = _(
            memeWithLikesAndVotingPowers.likesWithVotingPowersAndRewards,
        )
            .sortBy(
    ...
    private async calculateAndAssignRewardsTo3RandomTickets({
        ...
        const threeRandomTicketsIndexes: number[] = _.sortBy(
            uniqueRandoms(
                1,
                memeWithLikesAndVotingPowers.meme.favoritesCount,
                3,
            ),
        );
        ...
        for (const likeWithVotingPowerAndRewards of memeWithLikesAndVotingPowers.likesWithVotingPowersAndRewards) {
            ...
            if (
                ...
                likeWithVotingPowerAndRewards.rewardsDetailed.push({
                    for: rewardForDetail,
                    reward: rewardForCurrentRandomTicket,
                });
            }
    ...
    private async createStatusesAboutWinners(
        ...
        const memezatorOfficialAccount = await this.usersRepository.findByEthereumAddress(
            config.ADDRESS_OF_MEMEZATOR_OFFICIAL,
        );
    ...
    private async createStatusAboutWinner(
        ...
        const threeWinnerVoters =
            memeWithLikesAndVotingPowers.threeLikesWithVotingPowersAndRewardsWithBiggestRewards;

        let statusText =
            `**MEME №${place} OF ${dateFns.format(
                competitionStartDate,
                "yyyy/MM/dd",
            )}**\n` +
            `Voted: **${memeWithLikesAndVotingPowers.meme.favoritesCount}** votes\n` +
            `Prize: **${new Big(rewardPool * overallRewardFraction).toFixed(
                2,
            )}** PROM\n` +
            `\n  ` +
            `Author: ${this.getMarkdownLinkForUser(
                memeWithLikesAndVotingPowers.meme.author,
            )} ${new Big(memeWithLikesAndVotingPowers.rewardForAuthor).toFixed(
                2,
            )} PROM\n`;

        if (threeWinnerVoters[0]) {
            statusText += `Winner №1: **${new Big(
                threeWinnerVoters[0].reward,
            ).toFixed(2)}** PROM ${this.getMarkdownLinkForUser(
                threeWinnerVoters[0].like.user,
            )} (${threeWinnerVoters[0].votingPower} votes)\n`;
        }

        if (threeWinnerVoters[1]) {
            statusText += `Winner №2: **${new Big(
                threeWinnerVoters[1].reward,
            ).toFixed(2)}** PROM ${this.getMarkdownLinkForUser(
                threeWinnerVoters[1].like.user,
            )} (${threeWinnerVoters[1].votingPower} votes)\n`;
        }

        if (threeWinnerVoters[2]) {
            statusText += `Winner №3: **${new Big(
                threeWinnerVoters[2].reward,
            ).toFixed(2)}** PROM ${this.getMarkdownLinkForUser(
                threeWinnerVoters[2].like.user,
            )} (${threeWinnerVoters[2].votingPower} votes)\n`;
        }
    ...
    private async createTransactions(
        ...
        const inputsForTransactionsCreation: Array<{
            txnTo: string;
            txnSum: string;
        }> = [];
            ...
            inputsForTransactionsCreation.push(
                {
                    ...
                    txnSum: new Big(winners.firstPlace.rewardForAuthor)
                        .round(18)
                        .toString(),
                },
                    ...
                    (likeWithVotingPowerAndReward) => ({
                        txnTo:
                            likeWithVotingPowerAndReward.like.user
                                .ethereumAddress,
                        txnSum: new Big(likeWithVotingPowerAndReward.reward)
                            .round(18)
                            .toString(),
                    }),
                ),
            ...
            inputsForTransactionsCreation.push(
                {
                    ...
                    txnSum: new Big(winners.secondPlace.rewardForAuthor)
                        .round(18)
                        .toString(),
                },
            ...
            inputsForTransactionsCreation.push(
                {
                    ...
                    txnSum: new Big(winners.thirdPlace.rewardForAuthor)
                        .round(18)
                        .toString(),
                },
